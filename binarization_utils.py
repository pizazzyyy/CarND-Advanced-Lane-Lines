...
HLS_th_min = np.array([15, 0, 90])
HLS_th_max = np.array([100, 255, 255])
...
def thresh_frame_in_HLS(frame, min_values, max_values, verbose=False):

    HLS = cv2.cvtColor(frame, cv2.COLOR_BGR2HLS)

    min_th_ok = np.all(HLS > min_values, axis=2)
    max_th_ok = np.all(HLS < max_values, axis=2)
...
def binarize(img, verbose=False):
    ...
    if verbose:
        ...
        ax[0, 0].set_axis_off()
        ax[0, 1].imshow(sobel_mask, cmap='gray')
        ax[0, 1].set_title('sobel binary')
        ax[0, 1].set_axis_off()
        ...
        #ax[1, 1].set_axis_off()
        ax[1, 0].imshow(HLS_mask, cmap='gray')
        ax[1, 0].set_title('hls binary')
        ax[1, 0].set_axis_off()        
        ax[1, 1].imshow(binary, cmap='gray')
        ax[1, 1].set_title('before close')
        ax[1, 1].set_axis_off()
        ax[2, 0].imshow(eq_white_mask, cmap='gray')
        ax[2, 0].set_title('equalization')
        ax[2, 0].set_axis_off()
        ax[2, 1].imshow(closing, cmap='gray')
        ax[2, 1].set_title('after close')
        ax[2, 1].set_axis_off()
        plt.show()
